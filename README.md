...
“虫宿博物志”是一套面向儿童自然教育、乡村美育与公众科学体验的软硬件原型。Arduino UNO 采集观察点的温度、湿度和相对光照，Windows 本地服务负责串口、数据、AI、语音与图片合成，微信小程序完成拍摄、观察、作品创作和城乡交流。
...
### 后端已经启动，但小程序仍显示“本地后端未连接”

先在电脑浏览器访问 <http://127.0.0.1:3150/api/v1/health>；看到 `"status":"ok"` 说明后端正常。设置页在未连接时会每 2 秒自动重试，恢复后会自动切换成“本地服务正常”。如果一直保留旧提示，请在微信开发者工具中重新编译；仍未恢复时进入设置页点击“恢复本机默认地址”，确保检查地址为 `http://127.0.0.1:3150/api/v1/health`。

微信开发者工具模拟器可以访问电脑的 `127.0.0.1`；手机真机中的 `127.0.0.1` 指向手机自身，真机调试必须改用电脑局域网 IP。
